...
export default function CreateStorePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetchSuperAdminAPI('/stores', {
        method: 'POST',
        body: JSON.stringify({
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8">
      ...
      <div className="flex items-center gap-4">
        <Link href="/system/stores" className="p-2 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Create New Store</h1>
          <p className="text-slate-500 dark:text-slate-400 mt-1">Provision a new store store on the platform</p>
        </div>
      </div>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="pt-4 border-t border-slate-100 dark:border-slate-700 flex justify-end gap-4">
            <Link href="/system/stores" className="px-6 py-3 text-sm font-semibold text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 transition-all">
              Cancel
            </Link>
            <button
              type="submit"

...
async function handleFail(request: NextRequest, data: any = {}) {
  try {
    ...
    await fetch(`${backendUrl}/billing/complete/fail?tran_id=${tran_id}`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.redirect(`${origin}/admin/settings/billing/fail?tran_id=${tran_id}`, 303);

  } catch (error) {
    console.error("Billing Fail Route Error:", error);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error("Failed to parse POST body in fail route:", err);
  }
  return handleFail(request, data);
}

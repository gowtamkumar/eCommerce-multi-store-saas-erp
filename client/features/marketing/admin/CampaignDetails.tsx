'use client';
import { fetchCampaignLogs } from '@/services/campaign';
...
export default function CampaignDetails({ campaign, onClose }: CampaignDetailsProps) {
    ...
    useEffect(() => {
        const loadLogs = async () => {
            setLoadingLogs(true);
            try {
                const res = await fetchCampaignLogs(campaign.id, page, limit);
                if (res.success) {
            ...
            >
                ...
                <div className="p-8 bg-slate-900 text-white flex items-center justify-between">
                    <div className="space-y-1">
                        <div className="flex items-center gap-3">
                            <h2 className="text-3xl font-black tracking-tighter uppercase">{campaign.name}</h2>
                            <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${getStatusStyles(campaign.status)}`}>
                                {campaign.status}
                            </span>
                        </div>
                        <p className="text-slate-400 text-xs font-bold uppercase tracking-widest flex items-center gap-2">
                            <Info className="w-3 h-3" /> Campaign ID: {campaign.id}
                        </p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-8 overflow-y-auto max-h-[calc(90vh-120px)] custom-scrollbar space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        {[
                            { label: 'Total Audience', value: campaign.totalAudience, icon: Users, color: 'text-brand-600' },
                            { label: 'Successfully Sent', value: campaign.sentCount, icon: CheckCircle2, color: 'text-emerald-600' },
                            { label: 'Failed Delivery', value: campaign.failedCount, icon: AlertCircle, color: 'text-rose-600' },
                            { label: 'Target Groups', value: [campaign.targetUsers && 'Users', campaign.targetSubscribers && 'Subscribers', campaign.targetLeads && 'Leads'].filter(Boolean).length, icon: Info, color: 'text-blue-600' },
                        ].map((stat, i) => (
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="bg-white dark:bg-slate-800 p-8 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm space-y-6">
                                ...
                                <div className="space-y-3">
                                    {[
                                        { label: 'Registered Users', active: campaign.targetUsers },
                                        { label: 'Newsletter Subscribers', active: campaign.targetSubscribers },
                                        { label: 'Marketing Leads', active: campaign.targetLeads },
                                    ].map((target, i) => (
                            ...
                            <div className="bg-slate-900 text-white p-8 rounded-[2rem] space-y-6">
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <p className="text-sm font-bold">{dayjs(campaign.createdAt).format('MMM D, YYYY hh:mm A')}</p>
                                    </div>
                                    <div>
                                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Last Schedule Run</p>
                                        <p className="text-sm font-bold">
                                            {campaign.scheduleTime
                                                ? dayjs(campaign.scheduleTime).format('MMM D, YYYY hh:mm A')
                                                : 'Manual Execution'
                                            }
                                        </p>
                                    </div>
                                    ...
                                    logs.map((log) => (
                                        <tr key={log.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                                            <td className="px-8 py-4">
                                                <div className="font-bold text-slate-900 dark:text-white">{log.recipient?.name || 'Anonymous User'}</div>
                                                <div className="text-[10px] text-slate-400 font-medium">{log.recipient?.email || 'No Email'}</div>
                                            </td>
                                            <td className="px-8 py-4">

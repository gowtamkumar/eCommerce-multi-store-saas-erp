 ...
 * Receives stable callbacks from the parent — only re-renders when its own
 * item data changes, not when a sibling item's quantity changes.
 */
const CartItem = memo(function CartItem({ item, index, onUpdateQuantity, onRemove }: CartItemProps) {
    return (
        <div className="bg-white dark:bg-slate-800 rounded-3xl p-5 sm:p-7 shadow-sm border border-slate-100 dark:border-slate-700 flex flex-col sm:flex-row gap-6 hover:shadow-md transition-shadow group">
            {/* Product Image */}
            <div className="relative w-full sm:w-32 h-48 sm:h-32 flex-shrink-0 bg-slate-50 dark:bg-slate-900 rounded-2xl overflow-hidden border border-slate-100 dark:border-slate-700">
                {item.product.image ? (
                    <Image
                        ...
                        fill
                        // Only the first image gets priority — the rest lazy-load
                        priority={index === 0}
                        unoptimized
            ...
            <div className="flex-1 flex flex-col justify-between py-1">
                <div className="flex justify-between items-start gap-4">
                    <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-3">
                            <Price
                                amount={item.pricing?.final_price ?? item.line_total / item.quantity}
                                className="text-lg font-black text-blue-600"
                            />
                            {(item.pricing?.base_price ?? 0) > (item.pricing?.final_price ?? 0) && (
                                <Price
                                    amount={item.pricing!.base_price}
                                    className="text-sm text-slate-400 line-through font-medium"
                                />
                            )}
                        </div>
                    </div>
                ...
                <div className="flex justify-between items-end mt-6">
                    <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-900 rounded-xl p-1.5 border border-slate-200/50 dark:border-slate-800">
                        ...
                        <span className="text-sm font-black w-8 text-center text-slate-900 dark:text-white font-mono">
                            {item.quantity}
                        </span>
                        <button
...
export default function Cart() {
    ...
    // CartContext should already provide stable references via useCallback;
    // passing them directly avoids unnecessary wrapper layers.
    const handleUpdateQuantity = updateQuantity;
    const handleRemove = removeItem;

    // Memoize summary fallback — dep is [cart] (the stable source),
    // not cart?.summary which is an object compared by reference and always 'new'
    const summary = useMemo(
        () => cart?.summary ?? { subtotal: 0, offer_discount: 0, coupon_discount: 0, tax: 0, payable: 0 },
        [cart]
    );
                            ...
                            <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] p-8 shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-700 sticky top-28 overflow-hidden">
                                ...
                                <div className="space-y-5 mb-8">
                                    ...
                                    <div className="flex justify-between items-center text-slate-500 dark:text-slate-400">
                                        ...
                                        <span className="text-[10px] font-black uppercase tracking-widest bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded-md">
                                            Calculated later
                                        </span>
                                    </div>
                                </div>

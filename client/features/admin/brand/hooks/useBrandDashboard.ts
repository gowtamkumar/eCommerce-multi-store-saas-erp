...
export function useBrandDashboard() {
  ...
  const buildEndpoint = useCallback((page: number) => "/brands/stats", []);
  ...
  const handleDelete = (id: string) => {
    setConfirmModal({
      ...
      title: "Delete Brand",
      message:
        "Are you sure you want to delete this brand? Products associated with this brand will be affected.",
      onConfirm: async () => {
        const result = await mutate(
          `/brands/${id}`,
          { method: "DELETE" },

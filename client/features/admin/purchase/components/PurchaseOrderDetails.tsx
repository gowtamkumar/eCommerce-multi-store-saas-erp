...
import {
    ...
    History, Landmark, Package, Plus, Truck, Download
} from 'lucide-react';
...
import { useDownloadPO } from '@/lib/handleDownloadPO';

export default function PurchaseOrderDetails() {
    const { formatPrice } = useSettings();
    const { downloadPO } = useDownloadPO();
    const {
        order,
        loading,
        isPaymentModalOpen,
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-700">
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className="px-6 py-3 border-2 border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 hover:border-indigo-500 hover:text-indigo-500 rounded-2xl font-black transition-all hover:scale-[1.02] active:scale-95 flex items-center gap-2"
                    >

...
export function usePurchaseOrderDashboard() {
    ...
    useEffect(() => {
        void fetchOrders(1, debouncedSearch, statusFilter);
    ...
    const handleReceive = useCallback(async (id: string) => {
        ...
        try {
            ...
            void fetchOrders(pagination.page, debouncedSearch, statusFilter);
        } catch (error) {
            console.error(error);
    ...
    const handlePageChange = useCallback((newPage: number) => {
        if (newPage >= 1 && newPage <= pagination.totalPages) {
            void fetchOrders(newPage, debouncedSearch, statusFilter);
        }

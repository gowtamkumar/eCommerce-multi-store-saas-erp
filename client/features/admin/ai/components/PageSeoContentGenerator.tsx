...
import type { PageSeoResult } from "../types/ai-studio";

interface PageSeoContentGeneratorProps {
  loading: boolean;
  disabled: boolean;
  onGenerate: (payload: {
    pageTitle: string;
...
export function PageSeoContentGenerator({
  loading,
  disabled,
  onGenerate,
}: PageSeoContentGeneratorProps) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      >
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="font-bold text-slate-900 dark:text-white">Page SEO</h3>
        </div>
        <p className="text-sm text-slate-500 -mt-2">
          Generate meta title and description for a storefront page from its title and optional keywords.
        </p>
        ...
        <button
          type="submit"
          disabled={disabled || loading || !pageTitle.trim()}
        ...
        {!result ? (
          ...
          <>
            ...
            <p className="text-xs text-slate-500 pt-2">
              Copy into a page, or open the{" "}
              <Link href="/admin/pages" className="text-brand-600 font-semibold hover:underline">
                page builder
              </Link>{" "}
              to apply SEO on a live page.
            </p>
          </>
        )}
      </div>

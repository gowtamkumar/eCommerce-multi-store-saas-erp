...
export function useWarehouseStockReport() {
  ...
  const fetchStockReport = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const targetWarehouses = selectTargetWarehouses(
        warehouses,
        selectedBranchId,
        selectedWarehouseId,
      );
  ...
  const stats = useMemo(
    () => computeStockStats(filteredProducts),
    [filteredProducts],
  );
  ...
  const filterCounts = useMemo(
    () => [
      { key: "all" as const, label: "All Products", count: products.length },
      {
        ...
        label: "In Stock",
        count: products.filter((p) => !p.lowStock && !p.outOfStock).length,
      },
      {
        key: "lowStock" as const,
        label: "Low Stock",
        count: products.filter((p) => p.lowStock).length,
      },
      {
        key: "outOfStock" as const,
        label: "Out of Stock",
        count: products.filter((p) => p.outOfStock).length,
      },
    ],
    [products],
  );

...
export function useMarketingReport() {
    ...
    const loadMarketingData = useCallback(async () => {
        ...
        try {
            ...
            if (couponsRes.success) setCoupons(couponsRes.data?.coupons || couponsRes.data || []);
            if (subscribersRes.success) setSubscribersCount(subscribersRes.meta?.total || subscribersRes.total || 0);
            setOrders(allOrders);
    ...
    const financialDiscounts = useMemo<MarketingDiscountTotals>(() => {
        ...
        orders.forEach((order) => {
            couponDiscountTotal += parseFloat(String(order.couponDiscountAmount)) || 0;

            if (Array.isArray(order.items)) {
                order.items.forEach((item) => {
                    const itemPromoDiscount = parseFloat(String(item.discountAmount)) || 0;
    ...
    const metrics = useMemo<MarketingMetrics>(() => {
        ...
        return {
            ...
            totalPointsHeld: customers.reduce((sum, cust) => sum + (cust.loyaltyPointsBalance || 0), 0),
        };

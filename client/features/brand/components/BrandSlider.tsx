...
export default function BrandGrid({
  ...
  // Unique ID so scoped CSS doesn't bleed across multiple instances
  const uid = useRef(`bg-${Math.random().toString(36).slice(2, 7)}`).current;
    ...
    <div className="w-full">
      <div className="w-full">
        <div className="w-full">
          ...
          {/* ── Slider navigation controls — only shown for slider layout ── */}
          {isSlider && displayedBrands.length > 0 && (
            <div className={`mb-4 sm:mb-6 flex items-center ${
              styles?.textAlign === 'center' ? 'justify-center' :
              styles?.textAlign === 'right' ? 'justify-start' : 'justify-end'
            }`}>
              <div className="flex gap-2 sm:gap-3">
                <button
                  onClick={slideLeft}
                  className="w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 rounded-lg sm:rounded-xl border flex items-center justify-center transition-all duration-300 hover:scale-105 active:scale-95"
                  style={{
          ...
          {/* Scoped responsive CSS — keyed by uid so multiple instances don't clash */}
          <style>{`
            .${uid}-item {
              /* Mobile: mobileColumns per row */
              width: calc(${100 / mobileColumns}% - ${(16 * (mobileColumns - 1)) / mobileColumns}px);
            }
            @media (min-width: 640px) {
              .${uid}-item {
                /* sm: midpoint between mobile and desktop columns */
            ...
            >
              {[...Array(count)].map((_, i) => (
                <div
                  key={i}
                  className={isSlider
                    ? `${uid}-item shrink-0 aspect-[3/2] bg-slate-200 dark:bg-slate-800 animate-pulse`
              ...
              <motion.div ref={carouselRef} className="cursor-grab active:cursor-grabbing overflow-hidden">
                ...
                >
                  {displayedBrands.map((brand) => (
                    <motion.div key={brand.id} className={`${uid}-item shrink-0`}>
                      <Link

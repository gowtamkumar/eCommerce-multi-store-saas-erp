...
import { createPlatformCampaign, updatePlatformCampaign, PlatformCampaign, CampaignType } from '@/services/superAdminCampaign';
import { AnimatePresence, motion } from 'framer-motion';
import { Calendar, Mail, MessageSquare, Save, Send, X } from 'lucide-react';
...
interface PlatformCampaignFormProps {
    campaign?: PlatformCampaign | null;
    onClose: () => void;
    onSuccess: () => void;
}

export default function PlatformCampaignForm({ campaign, onClose, onSuccess }: PlatformCampaignFormProps) {
    const [loading, setLoading] = useState(false);
    const [activeTab, setActiveTab] = useState<CampaignType>(campaign?.type || 'EMAIL');
    const [formData, setFormData] = useState({
        name: campaign?.name || '',
        subject: (campaign as any)?.messages?.[0]?.subject || '',
        htmlContent: (campaign as any)?.messages?.[0]?.htmlContent || '',
        text: (campaign as any)?.messages?.[0]?.text || '',
        title: (campaign as any)?.messages?.[0]?.title || '',
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const res = isEdit
                ? await updatePlatformCampaign(campaign.id, payload)
                : await createPlatformCampaign(payload);

            if (res.success) {
                toast.success(isEdit ? 'Platform campaign updated' : 'Platform campaign created successfully');
                onSuccess();
            } else {
                toast.error(res.message || 'Failed to save platform campaign');
            }
        } catch (error: any) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
                ...
                <div className="px-8 py-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                    <div>
                        <h2 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight uppercase">
                            {isEdit ? 'Edit Platform Campaign' : 'Create Platform Campaign'}
                        </h2>
                        <p className="text-xs text-slate-500 font-medium tracking-wide">
                            {isEdit ? 'Refine platform campaign details' : 'Draft system announcement or marketing messages'}
                        </p>
                    </div>
                ...
                <div className="flex-1 overflow-y-auto p-8">
                    <form id="platformCampaignForm" onSubmit={handleSubmit} className="space-y-8">
                        {/* Channel Selection */}
                        {!isEdit && (
                            <div className="grid grid-cols-3 gap-4">
                                {[
                                    ...
                                    { type: 'PUSH', icon: Send, label: 'Push' },
                                ].map((item) => (
                                    <button
                                        key={item.type}
                                        type="button"
                                        onClick={() => setActiveTab(item.type as CampaignType)}
                                        className={`flex flex-col items-center gap-3 p-6 rounded-3xl border-2 transition-all cursor-pointer ${activeTab === item.type
                                            ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/10 text-indigo-600 dark:text-white'
                                            : 'border-slate-100 dark:border-slate-800 text-slate-400 hover:border-slate-200 dark:hover:border-slate-700'
                                            }`}
                                    >
                                        <item.icon className={`w-8 h-8 ${activeTab === item.type ? 'text-indigo-500' : ''}`} />
                            ...
                            <div className="space-y-6">
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        className="w-full px-5 py-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-transparent focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-800 outline-none transition-all font-bold text-slate-900 dark:text-white"
                                    />
                                </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input
                                            ...
                                            className="w-full pl-14 pr-5 py-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-transparent focus:border-indigo-500 focus:bg-white dark:focus:bg-slate-800 outline-none transition-all font-bold text-slate-900 dark:text-white"
                                        />
                                    </div>
                                        ...
                                        {[
                                            ...
                                            <label
                                                ...
                                                className={`flex items-center justify-between p-4 rounded-2xl border-2 cursor-pointer transition-all ${(formData as any)[audience.id]
                                                    ? 'border-indigo-500 bg-indigo-50/50 dark:bg-indigo-900/10 text-indigo-650'
                                            ...
                                            >
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    />
                                                    <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${(formData as any)[audience.id] ? 'bg-indigo-500 border-indigo-500' : 'border-slate-300 dark:border-slate-700'
                                                        }`}>
                                        ...
                                        >
                                            <div className="space-y-2">
                                                ...
                                                <input
                                                    ...
                                                    placeholder="Don't miss our latest updates!"
                                                    className="w-full px-5 py-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-transparent focus:border-indigo-500 outline-none transition-all font-bold text-slate-900 dark:text-white"
                                                />
                                            </div>
                                        </motion.div>
                                    )}
                                    ...
                                    {activeTab === 'SMS' && (
                                        ...
                                        >
                                            ...
                                            <textarea
                                                ...
                                                placeholder="Enter your text message announcement..."
                                                className="w-full px-5 py-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-transparent focus:border-indigo-500 outline-none transition-all font-bold text-slate-900 dark:text-white resize-none"
                                            />
                                        ...
                                        >
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <textarea
                                                    ...
                                                    placeholder="Enter push notification body message..."
                                                    className="w-full px-5 py-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-transparent focus:border-indigo-500 outline-none transition-all font-bold text-slate-900 dark:text-white resize-none"
                                                />
                                            </div>
                                            <div>
                ...
                <div className="px-8 py-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-inherit flex items-center justify-between">
                    <button
                        type="button"
                        onClick={onClose}
                        className="px-6 py-4 text-xs font-black uppercase tracking-widest text-slate-400 hover:text-slate-600 transition-all cursor-pointer"
                    >
                        Cancel
                    </button>
                    <button
                        type="submit"
                        form="platformCampaignForm"
                        disabled={loading}
                        className="flex items-center gap-3 px-10 py-5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-[1.5rem] font-black uppercase tracking-widest text-xs transition-all shadow-xl shadow-indigo-500/20 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                    >
                        {loading ? (

...
export default function StoreAnalytics({ storeId, data, store: initialStore }: StoreAnalyticsProps) {
  ...
  useEffect(() => {
    const fetchFeatures = async () => {
      ...
      try {
        const res = await fetchSuperAdminAPI(`/super-admin/stores/${storeId}/features`);
        if (res.success) {
  ...
  const handleUpdateStatus = async (newStatus: string) => {
    ...
    try {
      const res = await fetchSuperAdminAPI(`/super-admin/stores/${storeId}/status`, {
        method: 'PATCH',
        body: JSON.stringify({ status: newStatus }),
      });
      if (res.success) {
        setCurrentStore((prev) => ({ ...prev, status: newStatus }));
  ...
  const handleToggleFeature = async (featureSlug: string, overrideValue: boolean | null) => {
    ...
    try {
      ...
      if (res.success) {
        const refetch = await fetchSuperAdminAPI(`/super-admin/stores/${storeId}/features`);
        if (refetch.success) {
  ...
  const styles = getStatusStyles(currentStore.status);
  ...
  const stats = [
    ...
    { label: 'Total Products', value: data.counts.products, icon: StoreIcon, color: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-500/20' },
    ...
    <div className="space-y-10 pb-12 antialiased">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 bg-white dark:bg-slate-800 p-8 rounded-[3rem] border border-slate-100 dark:border-slate-700 shadow-sm">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-3 mt-1.5 text-xs">
                <span className="flex items-center gap-1 text-slate-400 font-bold uppercase tracking-widest text-[10px]">
                  ...
                  {currentStore.subdomain}.host.local
                </span>
                {currentStore.primaryCustomDomain && (
                  <>
        ...
        <div className="flex gap-3 w-full lg:w-auto">
          <button
            onClick={() => handleUpdateStatus(currentStore.status === 'active' ? 'suspended' : 'active')}
          ...
          <a
            href={currentStore.primaryCustomDomain ? currentStore.primaryCustomDomain : `http://${currentStore.subdomain}.localhost:3000`}
            target="_blank"
          ...
          <div className="relative overflow-hidden p-8 bg-slate-950 rounded-[2.5rem] text-white shadow-xl border border-slate-900">
            ...
            <div className="relative space-y-8 flex flex-col justify-between h-full">
              <div>
                ...
                <h4 className="text-3xl font-black">{currentStore.subscriptionPlan?.name || 'Legacy Tier'}</h4>
                <p className="text-slate-450 text-[10px] font-black mt-1 uppercase tracking-widest">{currentStore.subscriptionBillingCycle || 'MONTHLY'} Billing</p>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center py-3.5 border-b border-white/5">
                  ...
                  <span className={`text-[10px] font-black uppercase tracking-widest ${currentStore.subscriptionStatus === 'ACTIVE' ? 'text-emerald-400' : 'text-amber-400'}`}>
                ...
                <div className="flex justify-between items-center py-3.5 border-b border-white/5">
                  ...
                  <span className="text-xs font-bold font-mono">
                    {currentStore.subscriptionStartsAt ? new Date(currentStore.subscriptionStartsAt).toLocaleDateString() : 'N/A'}
                  </span>
                </div>
                <div className="flex justify-between items-center py-3.5 border-b border-white/5">
                  <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">Renewal</span>
                  <span className="text-xs font-bold font-mono text-indigo-400">
                    {currentStore.subscriptionEndsAt ? new Date(currentStore.subscriptionEndsAt).toLocaleDateString() : 'PERPETUAL'}
                  </span>
                </div>
              </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] border border-slate-150 dark:border-slate-700 p-8 flex flex-col shadow-sm">
            <div className="flex justify-between items-center mb-8">
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Store partition usage</p>
              </div>
          ...
          <div className="bg-white dark:bg-slate-800 rounded-[2.5rem] border border-slate-150 dark:border-slate-700 p-8 flex flex-col shadow-sm">
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-850 rounded-2xl mt-4">
              ...
              <p className="text-xs font-bold text-slate-750 dark:text-slate-350">
                {new Date(currentStore.createdAt).toLocaleDateString(undefined, { month: 'long', day: 'numeric', year: 'numeric' })}
              </p>
            </div>
          </div>
        </div>

...
export type StoreHealthRiskLevel = "low" | "moderate" | "elevated" | "critical";

export interface StoreHealthNarrative {
  summary: string;
  riskLevel: StoreHealthRiskLevel;
  keySignals: string[];
  recommendedActions: string[];
}

const RISK_STYLES: Record<StoreHealthRiskLevel, string> = {
...
interface StoreHealthAiPanelProps {
  days: number;
}

export function StoreHealthAiPanel({ days }: StoreHealthAiPanelProps) {
  const { configured } = usePlatformAiConfig();
  const [loading, setLoading] = useState(false);
  const [narrative, setNarrative] = useState<StoreHealthNarrative | null>(null);

  const generate = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetchSuperAdminAPI("/super-admin/ai/generate/store-health-narrative", {
        method: "POST",
      ...
      toast.error("Failed to generate store health narrative");
    } finally {
      setLoading(false);
    }
  }, [days]);
    ...
    <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 p-6 shadow-sm">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-5">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-bold text-slate-900 dark:text-white">Store health insight</h2>
          </div>
        ...
        {configured ? (
          ...
          <p className="text-xs text-amber-800 dark:text-amber-200 max-w-xs">
            ...
            to enable store health narratives.
          </p>
        )}
      </div>
      ...
      {!narrative && !loading ? (
        <p className="text-sm text-slate-500 dark:text-slate-400">
          Uses billing, subscription status, engagement buckets, and {days}-day trends across all stores.
        </p>
      ) : null}

...
import { StatusStyles, StoreListProps, Store } from '../types/store.types';

type StoreWithExpiry = Store & { daysUntilExpiry?: number | null };
...
export default function StoreList({ initialStores }: StoreListProps) {
  ...
  const fetchStores = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetchSuperAdminAPI(`/super-admin/stores?${qs}`);
  ...
  useEffect(() => {
    const id = setTimeout(() => fetchStores(), 0);
  ...
  const toggleSelectAll = () => {
    if (selectedIds.size === stores.length) {
      ...
      setSelectedIds(new Set(stores.map(t => t.id)));
    }
  };
  ...
  const handleBulkAction = async () => {
    ...
    try {
      ...
      if (bulkAction === 'suspend') {
        await fetchSuperAdminAPI('/super-admin/stores/bulk-status', {
          method: 'POST',
  ...
  const columns: DataTableColumn<StoreWithExpiry>[] = [
    {
      key: 'select',
      header: (
        <input
    ...
    },
    {
      ...
      className: 'px-6 py-4',
      cell: (store) => {
        const styles = getStatusStyles(store.status);
        return (
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="font-black text-slate-900 dark:text-white leading-tight capitalize">{store.storeName}</p>
      ...
      cell: (store) => (
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-black uppercase tracking-wider text-slate-600 dark:text-slate-300">
            {store.subscriptionPlan?.name || 'Legacy Tier'}
          </span>
        </div>
      ),
    },
      ...
      cell: (store) => (
        <div className="flex flex-col gap-1">
          ...
          <span className="text-[8px] font-bold text-slate-400 uppercase tracking-widest ml-1">
            {store.subscriptionBillingCycle || 'MONTHLY'}
          </span>
        </div>
      ),
    },
      ...
      cell: (store) => {
        ...
        return (
          <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${styles.bg}`}>
            <span className={`w-1 h-1 rounded-full ${styles.dot} ${store.status === 'active' ? 'animate-pulse' : ''}`} />
            {store.status}
          </span>
        );
      },
    },
    {
      ...
      className: 'px-6 py-4',
      cell: (store) => {
        const isExpiringSoon = store.daysUntilExpiry !== null && store.daysUntilExpiry !== undefined && store.daysUntilExpiry >= 0 && store.daysUntilExpiry <= 7;
        const isExpired = store.daysUntilExpiry !== null && store.daysUntilExpiry !== undefined && store.daysUntilExpiry < 0;
        return (
          <div>
            <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
              {store.subscriptionEndsAt
                ? new Date(store.subscriptionEndsAt).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })
                : 'PERPETUAL'}
            </div>
    ...
    },
    {
      ...
      cell: (store) => (
        <div className="flex justify-end gap-2">
          <Link
            href={`/system/stores/${store.id}/analytics`}
          ...
          <a
            href={store.primaryCustomDomain ? store.primaryCustomDomain : `http://${store.subdomain}.localhost:3000`}
            target="_blank"
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400">
            {loading ? 'Loading...' : `${stores.length} stores`} in the cluster.
          </p>
        </div>
        <div className="flex flex-wrap gap-3">
          ...
          {/* Refresh */}
          <button onClick={fetchStores} disabled={loading} className="p-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 transition-all">
            <RefreshCw className={`w-4 h-4 ${loading ? 'animate-spin text-indigo-500' : 'text-slate-500'}`} />
          </button>
          {/* Create Store */}
          <Link
            href="/system/stores/create"
      ...
      <DataTable
        ...
        rowClassName={store => selectedIds.has(store.id) ? 'bg-indigo-50/40 dark:bg-indigo-950/10' : ''}
      />
    </div>
  );
}

...
This document outlines the architectural and functional guidelines for implementing the **Super Admin** role and its associated features in the eCommerce Multi-Store SaaS platform.

## 1. Overview
The Super Admin is the system-level administrator responsible for managing the entire platform infrastructure, monitoring store health, and handling billing/subscription life cycles across all merchants.

## 2. Access Control & Security
- **Role Identification**: The system uses `UserRole.SuperAdmin` (assigned in the `UserEntity`) to distinguish system admins from store admins.
- **Cross-Store Access**: Super Admin endpoints must explicitly bypass the standard `StoreGuard` to allow visibility into data across all `storeId` partitions.
...
- **Implementation**:
    - Add a `status` field to the `StoreEntity` (e.g., `enum { ACTIVE, SUSPENDED, DELETED }`).
    - Create a `PATCH /store-traffic/stores/:id/status` endpoint.
    - **Enforcement**: Update the generic `StoreGuard` or a Global Interceptor to check the store status. If a store is `SUSPENDED`, the API should return a `403 Forbidden` with a message like `"Store is temporarily suspended"`.
...
- **Implementation**:
    - Update the `plan` field in the `StoreEntity` or a dedicated `SubscriptionEntity`.
    - Create a `PATCH /store-traffic/stores/:id/plan` endpoint.
    - **Logic**: 
        ...
        - Re-evaluate feature flags for the store immediately.

### 3.4 See Traffic (Platform Analytics)
- **Objective**: Monitor the overall health and popularity of stores across the platform.
- **Implementation**:
    - **Option A (Internal Tracking)**: Implement a lightweight tracking service that logs requests or page views into a `TrafficLog` table (partitioned by `storeId`).
...
- **Isolation**: The Super Admin dashboard should be a separate route (e.g., `/store-traffic/*`) to avoid confusion with the standard merchant admin dash (`/admin/*`).
- **Global Search**: Implement a high-performance search across all stores and users.

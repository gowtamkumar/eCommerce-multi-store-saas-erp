...
export interface OrderCreationStrategy {
  resolveItems(dto: CreateOrderDto, context: OrderCreationContext, deps: OrderServiceDependencies): Promise<OrderItemEntity[]>;
  calculateTotals(order: OrderEntity, items: OrderItemEntity[], dto: CreateOrderDto, context: OrderCreationContext, deps: OrderServiceDependencies): Promise<void>;
}
